...
If `~/.config/llm-wiki/config.json` contains `registry_path`, read that registry
before ingesting, compiling, or routing material from outside an active wiki
repo. The registry can define multiple named hubs, boundaries, and first-run
scaffold policy.

On machines with both personal and work wikis, do not assume every wiki request
belongs in the default hub. Route first:

- personal/general agent workflows, LLM systems notes, homelab notes, and
  reusable non-work setup patterns go to the personal hub;
- work-specific architecture, team/domain knowledge, internal service context,
  and work agent-plugin notes go to the matching work hub;
- mixed notes should be split into sanitized general content and work-specific
  content, or escalated to the user when the split is unclear.

Use the local helper when deterministic routing is useful:

```bash
python3 scripts/wiki-router.py status
python3 scripts/wiki-router.py route --text "<candidate note>"
```
...
If the selected hub or topic is missing and the user wants to create it, scaffold
the standard topic-hub layout instead of assuming a legacy flat wiki:

```bash
python3 scripts/wiki-router.py scaffold-hub --path <hub-path> --topic <topic> --title "<Title>" --description "<Description>"
python3 scripts/wiki-router.py scaffold-topic --hub <hub-name> --topic <topic> --title "<Title>" --description "<Description>"
```

Scaffolding must be conservative: create only missing files and directories,
never overwrite existing content, and preserve any old wiki material for an
explicit migration.
...
4. **Dual-linking for Obsidian + OpenCode.** Cross-references use both `[[wikilink]]` (for Obsidian graph view) and standard markdown links (for OpenCode navigation) on the same line, for example a wikilink plus a path like `wiki/concepts/example.md`. Bidirectional when it makes sense.
