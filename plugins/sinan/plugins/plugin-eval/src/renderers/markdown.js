...
function renderBudgets(budgets) {
  ...
  if (budgets.explicit_only_deferred_cost_tokens) {
    lines.push(
      `- explicit_only_deferred_cost_tokens: ${budgets.explicit_only_deferred_cost_tokens.value} (${budgets.explicit_only_deferred_cost_tokens.band}, unscored diagnostic)`,
    );
  }
  if (budgets.unpackaged_text_tokens) {
    lines.push(
      `- unpackaged_text_tokens: ${budgets.unpackaged_text_tokens.value} (${budgets.unpackaged_text_tokens.band}, unscored diagnostic)`,
    );
  }
...
function renderBudgetExplanation(payload) {
  ...
  const whyBullets = [
    ...
    ...(budgets.invocation_policy?.explicit_only_skill_count > 0
      ? [
          `- Explicit-only skills are excluded from trigger, invoke, and scored deferred budgets, but their standalone load ceiling is shown separately.`,
        ]
      : []),
    ...(budgets.unpackaged_text_tokens
      ? [
          `- Unpackaged repo text is shown as a diagnostic and is not scored as active package surface.`,
        ]
      : []),
  ...
  return [
    ...
    section("Details", [
      ...
      budgets.explicit_only_invoke_cost_tokens
        ...
        : "",
      budgets.explicit_only_deferred_cost_tokens
        ? detailsBlock(
            "Explicit-only deferred components",
            budgets.explicit_only_deferred_cost_tokens.components.length > 0
              ? budgets.explicit_only_deferred_cost_tokens.components.map((component) => `- ${component.label}: ${component.tokens} tokens`).join("\n")
              : "- None",
          )
        : "",
      budgets.unpackaged_text_tokens
        ? detailsBlock(
            "Unpackaged text diagnostics",
            budgets.unpackaged_text_tokens.components.length > 0
              ? budgets.unpackaged_text_tokens.components.map((component) => `- ${component.label}: ${component.tokens} tokens`).join("\n")
              : "- None",
          )
        : "",
