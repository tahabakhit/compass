...
async function writePluginBudgetFixture(rootPath) {
  ...
  await fs.writeFile(path.join(rootPath, ".mcp.json"), '{"mcpServers":{}}\n', "utf8");
  await fs.mkdir(path.join(rootPath, "docs"), { recursive: true });
  await fs.writeFile(
    path.join(rootPath, "docs", "large-dev-note.md"),
    `# Large Dev Note\n\n${"Development-only repo text. ".repeat(3000)}\n`,
    "utf8",
  );
  ...
  await fs.mkdir(path.join(rootPath, "skills", "router", "references"), { recursive: true });
  await fs.writeFile(
    path.join(rootPath, "skills", "router", "references", "flow.md"),
    `# Router Flow\n\n${"Implicit support. ".repeat(40)}\n`,
    "utf8",
  );
  ...
  await fs.mkdir(path.join(rootPath, "skills", "specialist", "references"), { recursive: true });
  await fs.writeFile(
    path.join(rootPath, "skills", "specialist", "references", "deep-detail.md"),
    `# Specialist Detail\n\n${"Explicit-only support. ".repeat(120)}\n`,
    "utf8",
  );
}
...
test("plugin budget excludes explicit-only specialist skills from implicit active budget", async () => {
  ...
  const deferredLabels = result.budgets.deferred_cost_tokens.components.map((component) => component.label);
  const explicitLabels = result.budgets.explicit_only_invoke_cost_tokens.components.map((component) => component.label);
  const explicitDeferredLabels = result.budgets.explicit_only_deferred_cost_tokens.components.map((component) => component.label);
  const unpackagedLabels = result.budgets.unpackaged_text_tokens.components.map((component) => component.label);

  assert.equal(result.target.kind, "plugin");
  assert.equal(result.budgets.method, "estimated-static-package-and-policy-aware");
  ...
  assert.ok(deferredLabels.includes(".mcp.json"));
  assert.ok(deferredLabels.includes("skills/router/agents/openai.yaml"));
  assert.ok(deferredLabels.includes("skills/router/references/flow.md"));
  assert.ok(!deferredLabels.includes("skills/specialist/agents/openai.yaml"));
  assert.ok(!deferredLabels.includes("skills/specialist/references/deep-detail.md"));
  assert.ok(explicitDeferredLabels.includes("skills/specialist/agents/openai.yaml"));
  assert.ok(explicitDeferredLabels.includes("skills/specialist/references/deep-detail.md"));
  assert.ok(unpackagedLabels.includes("docs/large-dev-note.md"));
  assert.ok(result.budgets.unpackaged_text_tokens.value > result.budgets.deferred_cost_tokens.value * 10);
  assert.ok(!result.checks.some((check) => check.id === "deferred_cost_tokens-budget-high"));
  assert.ok(result.metrics.some((metric) => metric.id === "explicit_only_invoke_cost_tokens"));
  assert.ok(result.metrics.some((metric) => metric.id === "explicit_only_deferred_cost_tokens" && metric.band === "info"));
  assert.ok(result.metrics.some((metric) => metric.id === "unpackaged_text_tokens" && metric.band === "info"));
});
