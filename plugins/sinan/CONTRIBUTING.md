...
Open an issue on [GitHub](https://github.com/tahabakhit/ming/issues). Include:
...
3. Make your changes
4. Run the relevant verification command, usually `npm test`
5. Open a PR against `main`

### What to watch out for

**Cross-platform compatibility.** Sinan runs on Windows, macOS, and Linux. Before submitting:
...
**Plugin architecture.** Sinan is distributed as a Claude Code and Codex plugin. Users install it once and it works across projects.

- Hook scripts live in `hooks_src/` and are installed per-project via `scripts/install-hooks.js`
- The `init-project` SessionStart hook auto-scaffolds per-project state (`.planning/`, `.sinan/scripts/`)
...
7. Run the focused hook tests listed in `CLAUDE.md`
...
> **Migrating from copy-based install?** These hooks previously lived at `.claude/hooks/`. The paths in your `settings.local.json` need to use the `${CLAUDE_PLUGIN_ROOT}` form shown above.

## Code Style

- Node.js scripts use CommonJS (`require`), not ESM
- Keep hooks fast (under 5s for PreToolUse, under 30s for PostToolUse)
- Prefer non-blocking behavior for observability hooks. Blocking hooks need focused tests and clear user-facing failure output.
