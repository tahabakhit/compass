...
function generateConfigToml() {
  ...
  const entries = [mcpServerToToml('citadel-state', {
    ...
    _codex: {
      ...
      tool_timeout_sec: 30,
      required: false,
      instructions: 'Use citadel_status to orient on campaign, fleet, telemetry, and artifact state before invoking Sinan workflows.',
    },
  })];
...
function generatePluginManifest() {
  ...
  const repositoryUrl = repoUrl.replace(/^git\+/, '').replace(/\.git$/, '') || 'https://github.com/SethGammon/sinan';
  ...
  const longDescription = isSinan
    ? 'Sinan is a cross-project Codex and Claude Code harness for durable planning state, proportional routing, explicit specialist skills, lifecycle hooks, telemetry, and capped context snapshots.'
    : 'Sinan adds durable planning state, reusable skills, lifecycle hooks, telemetry, PR triage, and coordinated multi-agent workflows to Codex.';
...
function main() {
  ...
  console.log(`  Sinan root: ${CITADEL_ROOT}`);
