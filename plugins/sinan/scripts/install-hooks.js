...
/**
 * install-hooks.js — Resolves Sinan hook paths into a project's .claude/settings.json
 *
 * Why this exists:
 ...
 *
 * Usage:
 *   node /path/to/Sinan/scripts/install-hooks.js                            # from project dir
 *   node /path/to/Sinan/scripts/install-hooks.js /project                   # explicit project path
 *   node /path/to/Sinan/scripts/install-hooks.js --hook-profile latest      # force full hook set
 *   node /path/to/Sinan/scripts/install-hooks.js --claude-version 2.1.75    # test compatibility mode
 */
...
function main() {
  try {
    ...
    console.error('Is this script inside a Sinan installation?');
    process.exit(1);
  }
}
