...
description: >-
  Use when 4-phase root cause analysis: observe, hypothesize, verify, fix.
  Enforces investigation before any code changes. Emergency stop after 2
  failed fixes. Prevents shotgun debugging and fix cascades.
user-invocable: true
---
...
### Phase 2.5: SELF-CONSISTENCY CHECK

For non-trivial root cause diagnosis, invoke `/self-consistency-reasoner`
internally before editing:

1. Generate 5 independent root-cause paths from different starting points:
   symptom backward, data flow forward, dependency boundary, recent change, and
   configuration/runtime assumptions.
2. Aggregate the final root-cause answers by majority vote.
3. If confidence is `<=50%`, do not edit. Gather more evidence or ask for the
   missing product/runtime assumption.
4. If confidence is `60-99%`, proceed but note the minority hypothesis in the handoff.
...
- Non-trivial diagnoses use self-consistency before Phase 4 edits
