...
name: score-only
skill: improve
description: /improve sinan --score-only with existing rubric outputs a scorecard and makes no changes
tags: [happy-path]
input: /improve sinan --score-only
...
`with-completed-campaign` provides a `.planning/rubrics/sinan.md` rubric file.
The rubric already exists — Phase 0 should be skipped.

## Expected Behavior

1. Detects existing rubric at `.planning/rubrics/sinan.md` — skips Phase 0
